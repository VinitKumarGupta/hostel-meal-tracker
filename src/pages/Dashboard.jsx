    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="flex flex-col items-center space-y-4">
          ...
          <p className="text-gray-500 font-medium">Syncing meal tracker database...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-slate-50/50 pb-16 relative">
      ...
      <header className="sticky top-0 z-40 bg-white/80 backdrop-blur-md border-b border-gray-100 shadow-sm">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <span className="font-extrabold text-gray-900 tracking-tight text-lg sm:text-xl">Hostel Meal Tracker</span>
          </div>
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 mt-8 space-y-10">
        ...
        <section aria-label="Overview stats" className="space-y-4">
          <h2 className="text-xs font-bold text-gray-400 uppercase tracking-widest">Analytics Dashboard</h2>
        ...
        <section aria-label="Personal meal counts" className="space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-xs font-bold text-gray-400 uppercase tracking-widest">Your Meal Counter</h2>
            <span className="text-xs font-semibold text-indigo-600">{myProfile.name}</span>
          </div>
        ...
        <section aria-label="All roommates list" className="space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-xs font-bold text-gray-400 uppercase tracking-widest">All Roommates (Real-Time)</h2>
            <span className="text-xs text-gray-400">Updates live across all phones</span>
          </div>

          <div className="bg-white rounded-3xl p-6 border border-gray-100 shadow-xl shadow-gray-100/50 space-y-4">
            {roommates.length === 0 ? (
              <p className="text-center text-sm text-gray-400 py-6 font-medium">No roommates registered yet.</p>
            ) : (
              roommates.map((roommate) => (
                <UserCard
                  key={roommate.id}
                  user={roommate}
                  isCurrentUser={roommate.uid === currentUser?.uid}
                />
              ))
            )}
          </div>
